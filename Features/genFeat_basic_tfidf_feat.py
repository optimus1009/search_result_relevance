...
def extract_feat(path, dfTrain, dfTest, mode, feat_names, column_names):
	...
	## all_text = query + title + description
    ...
    if vocabulary_type == "common":
        ...
        vocabulary = None
    for feat_name,column_name in zip(feat_names, column_names): # feat_names = ["query", "title", "description"] column_names = [ "query", "product_title", "product_description" ]
	...
	# 以下代码以query/title/description顺序堆叠矩阵
    for i,feat_name in enumerate(feat_names):
        ...
        if i == 0:
			...
			# 这里我们使用的vstack()函数是scipy工具包中sparse模块的一个函数
			# scipy.sparse.vstack(blocks, format=None, dtype=None)
			# Stack sparse matrices vertically (row wise) (即把稀疏矩阵以垂直方向堆叠)
...
if __name__ == "__main__":
    ...
    stats_feat_num = len(quantiles_range) + len(stats_func)  # 5

    ## tfidf config
    vec_types = [ "tfidf", "bow" ]
    ngram_range = config.basic_tfidf_ngram_range  # basic_tfidf_ngram_range=(1,3)
    vocabulary_type = config.basic_tfidf_vocabulary_type #  common individual
    ...
    def cat_text(x):
        ...
        return res
    dfTrain["all_text"] = list(dfTrain.apply(cat_text, axis=1)) # apply()函数的返回值是其参数函数的返回值
    ...
    for vec_type in vec_types:  
		...
		# n_runs = 3
        for run in range(config.n_runs):  # run -> 0, 1, 2 
