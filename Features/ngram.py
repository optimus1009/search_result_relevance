...
def getUnigram(words):
    """
        Input: a list of words, e.g., ['I', 'am', 'xionglin']
...
def getBigram(words, join_string, skip=0):
	"""
	   Input: a list of words, e.g., ['I', 'am', 'xionglin']
	   Output: a list of bigram, e.g., ['I_am', 'am_xionglin']
	   I use _ as join_string 
	"""
	## 这里使用一个断言语句，确保输入的类型正确
	## tips：range(m,n) 返回一个k,  m=<k<n
	assert type(words) == list
	L = len(words)
	if L > 1:
		lst = []
		for i in range(L-1):
			for k in range(1,skip+2):  #这里的k值实际为1
				if i+k < L:
...
def getTrigram(words, join_string, skip=0):
	"""
	   Input: a list of words, e.g., ['I', 'am', 'xionglin']
	   Output: a list of trigram, e.g., ['I_am_xionglin']
	   I use _ as join_string 
	"""
	assert type(words) == list
	L = len(words)
