...
def extract_feat(df):
    ...
    print "output unigram 1-9"    
    print df["title_unigram"].[0:10]
    ...
    for feat_name in feat_names:
        for gram in grams:
			...
			# 这里使用map()函数，因为该函数能够返回一个列表，map(func,seq1[,seq2...]):将函数func作用于给定序列(s)的每一个元素
			# 并用一个列表来提供返回值，如果func为NOne,func表现为一个身份函数，返回一个含有每个序列中元素集合的n个元祖的列表
...
if __name__ == "__main__":
	...
	## stratified_label = "query"    data_folder = "../../Data"
    ...
    for run in range(config.n_runs):
		...
		## 交叉检验的过程实际上是把实验重复做K次，每次实验都从K个部分中选择一个不同的部分作为测试数据,
		## 保证K个部分的数据都分别做过测试数据），剩下的K-1个当作训练数据进行实验，最后把得到的K个实验结果平均
